				...
				<c:forEach var="dto" items="${list}">			
					<c:if test="${mov.mov_title eq dto.mov_title && fn:substring(dto.show_date,0,10) eq date}">
						...
						<c:if test="${date eq today && time > dto.show_time}">
							<button style='width:80px;margin: 0 5px 7px 0' disabled='disabled' class='btn btn-outline-dark'>
								${dto.show_time }<br>
								<span style='font-size: 8px;margin:0;height: 15px;color:gray'>${dto.room_name }</span>
							</button>
						</c:if>
						<c:if test="${date != today || time <= dto.show_time}">
							<button style='width:80px;margin: 0 5px 7px 0' onclick="resCheck(this,'${s_id}','${dto.show_code }')"class='btn btn-outline-dark'>
								${dto.show_time }<br>
								<span style='font-size: 8px;margin:0;height: 15px;color:gray'>${dto.room_name }</span>
							</button>
						</c:if>
